# xray-mini-app-assets implementation 0002 instruction
...
| Human request to npm-link `@xray-network/xray-js`, remove direct legacy dependencies, and reuse its utilities | `LOCAL` | Yes      | Requires host communication, Cardano types, asset decoding, and slot constants to resolve through the sibling runtime. |
| `../xray-js/packages/runtime` public exports                                                                  | `LOCAL` | Yes      | Defines the mini-app bridge, Cardano types, asset/encoding helpers, and configuration constants.                       |
...
| `C05`     | Align preferences, live host state, routing effects, Ant Design escape access, and app types with template boundaries; npm-link `xray-js` as the sole bridge/Cardano facade and replace duplicate CIP-67, asset/encoding, and config utilities with its public exports. | Preserve the persisted preference key, asset behavior, tolerant invalid-label fallback, and label presentation; host-owned values must not enter persistence. | Manifest/lockfile and `app/{integrations,shared,store,theme,types,utils}` | Inspect sibling resolution, typecheck/build, and scan for direct legacy dependencies, duplicate CRC/CIP-67 helpers, legacy app state, effects, package/provider names, and CW3 terminology. |
...
7. Replace the mixed persisted/runtime app store and monolithic effects wrapper with template-aligned preferences, SDK hooks, and routing modules.
...
- Scan for obsolete `store/app`, `effects.tsx`, and `utils/escapeAntd` ownership.
...
- Confirm no direct mini-app SDK, Buffer, or Koios dependency/import remains.
- `git diff --check`
...
preferences, live SDK state, and routing effects follow the template approach; required validation
passes and application-specific behavior remains.

## Out of scope

Redesigning pages, changing asset behavior, or removing unrelated feature-specific utilities.
