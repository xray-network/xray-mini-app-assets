...
| `C05`     | `IMPLEMENTED` | Split standalone preferences from bridge-owned live state; npm-linked `xray-js`; routed bridge hooks/types, Cardano types/constants, cardano-lib-backed CIP-67 validation, and asset/hex decoding through its public subpaths; removed the local CRC-8 codec plus direct mini-app SDK and Buffer dependencies; separated routing effects; and moved Ant Design escape access into `theme`. | Exact sibling resolution, dependency/source scans, CIP-67 vector smoke, typecheck, production build, and legacy architecture/package/terminology/CRC scans passed. |
