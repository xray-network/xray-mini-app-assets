# xray-mini-app-assets implementation 0002 result

Result-Version: v1
Implementation-ID: xray-mini-app-assets/0002
...
| `C05`     | `IMPLEMENTED` | Split standalone preferences from bridge-owned live state; npm-linked `xray-js`; consolidated app-local settings adapters under `app/integrations/xray-js`; routed bridge hooks/types, Cardano types/constants, cardano-lib-backed CIP-67 validation, and asset/hex decoding through its public subpaths; removed the local CRC-8 codec plus direct mini-app SDK and Buffer dependencies; separated routing effects; and moved Ant Design escape access into `theme`. | Exact sibling resolution, integration-tree/dependency/source scans, CIP-67 vector smoke, typecheck, production build, and legacy architecture/package/terminology/CRC scans passed. |

## Outcome

The assets frontend now follows the React template's theme, state, integration, routing, type, and utility ownership architecture.

## Inputs consumed

The human implementation request, the local React template theme/style modules, and the assets frontend source and manifest.

## Project changes

Introduced shared theme modules and XRAY bridge effective settings; migrated CSS/Tailwind; replaced the mixed app store and monolithic effects wrapper with preferences, xray-js bridge hooks, and routing modules; reused xray-js asset/encoding/config exports; updated root providers and npm metadata.
...
| `C05`     | Persistence contains standalone preferences only while xray-js bridge hooks own live host state; xray-js also owns shared Cardano types, constants, CIP-67 validation, and generic asset/encoding helpers. | The existing storage key, asset behavior, tolerant invalid-label fallback, and label presentation are preserved; host values are not persisted. | Use xray-js public subpaths for shared runtime behavior and keep only UI-specific label presentation local. |
...
The human expanded this non-terminal implementation to include the template's application-module structure and xray-js consolidation. The later focused `xray-js/cardano` CIP-67 export removed the final local codec; only tolerant UI adaptation and label presentation remain local.
...
Visually review representative assets pages in standalone light/dark/system modes and inside an XRAY host.
